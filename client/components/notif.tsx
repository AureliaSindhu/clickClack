...
import { Camera, ImageIcon, Palette, Share2, X, Instagram } from "lucide-react";
...
export default function GetStarted() {
    ...
    const [notifications, setNotifications] = useState<Notification[]>([
        ...
        { id: 4, title: "Don't forget to tag @aacode on Instagram!", icon: Instagram, color: "bg-[var(--canvas-dark)]" },
    ]);
    ...
    return (
        ...
        <div className="space-y-4">
            <div className="p-4 text-black text-center">
                <h1 className="text-2xl font-semibold font-chillax">Notification Center</h1>
                <p className="text-sm opacity-75">{notifications.length} new notifications</p>
            </div>
            {notifications.map((notif, index) => (
                <div key={notif.id} className="rounded-xl shadow-lg overflow-hidden">
                    <div className="flex items-center p-4 bg-[var(--charcoal)] bg-opacity-80 relative">
                    <div
                        className={`flex-shrink-0 h-10 w-10 rounded-full ${notif.color} flex items-center justify-center`}
                    >
                        <notif.icon className="h-6 w-6 text-white" />
                    </div>
                    <div className="ml-4 flex-grow">
                        <p className="text-sm font-medium text-white">{notif.title}</p>
                        {/* Show step indicator only for the first three notifications */}
                        {notif.id !== 4 && (
